...
Known gap, not yet investigated: now that `packages/server/.env` exists locally with a real `OPENAI_API_KEY`, `tests/evaluation/`'s eval suite no longer self-skips — a `bun test` run took ~10 minutes (vs. the usual ~2s for `tests/routes/`) and had 12 failures, almost certainly the eval suite making real API calls. If you need a fast/deterministic `bun test`, run `EVAL_MODEL=openai bun test tests/routes` — the `EVAL_MODEL=openai` prefix is required because the local `.env` sets `EVAL_MODEL=gemini-2.5-flash`, which routes `chatRoutes.ts` through the unmocked Gemini SDK and causes all chat tests to fail even though they don't actually call the AI.

`fakeDb.select`'s `from` result now exposes both an `innerJoin` branch (for queries that JOIN the `user` table) and a `where` branch (for non-JOIN queries) — both resolve through `dbSelectWhereResult` or the `orderBy/limit` chain as normal. The `innerJoin` mock result also exposes a `leftJoin` branch (added when `GET /alerts/history` and `GET /alerts/:id` gained the clinician double-join), so routes with the chain `.innerJoin(patient).leftJoin(clinician).where()` work through the same `dbSelectWhereResult`/`dbSelectOrderByResult` mocks without any new mock exports. Non-JOIN routes and single-JOIN routes are unaffected.
...
bun run digest                   # scripts/daily-digest.ts — send open-alert digest email (NODE_ENV=production, hits Railway DB)
```
...
- **Routing:** React Router DOM in `src/shared/routing/routes.tsx`. Authenticated pages are wrapped by `PrivateRoutes.tsx`. Clinician-only pages are additionally wrapped by `ClinicianRoute.tsx` (nested inside `PrivateRoutes`) — it reads `user.role` from `useAuth()` and redirects non-clinicians to `/`; it does not re-render the NavBar since `PrivateRoutes` already does that. Currently the only clinician-only route is `/alerts` → `AlertQueuePage.tsx`.
...
- **Entry:** `index.ts` creates an Express app and mounts seven per-domain routers under `/api`: `routes/authRoutes.ts` (signup/login/refresh-token/logout), `routes/profileRoutes.ts`, `routes/checkinRoutes.ts`, `routes/goalRoutes.ts`, `routes/wellnessRoutes.ts`, `routes/chatRoutes.ts` (conversations + the chat endpoint), and `routes/alertRoutes.ts` (clinician-only alert review queue). Each file only imports the middleware/schemas it actually uses — there is no longer a single catch-all `userRoutes.ts`.
...
- **Alert routes (`alertRoutes.ts`):** five routes in this fixed order (order matters — `count` and `history` must come before `/:id` or Express's param capture swallows them):
  1. `GET /alerts` — open alerts only, JOINed to `user` table for `patientName`/`patientEmail`; ordered by `createdAt DESC`.
  2. `GET /alerts/count` — `{ count: N }` of open alerts; used by NavBar badge.
  3. `GET /alerts/history` — acknowledged + resolved alerts, JOINed to `user` for patient info and LEFT JOINed to a `clinicianAlias = alias(userTable, "clinician")` for `acknowledgedBy`; ordered by `updatedAt DESC`. `acknowledgedBy` in the response is the clinician's email (resolved from the FK UUID at read time) — the DB column still stores the UUID FK, only the SELECT projection changes.
  4. `GET /alerts/:id` — single alert detail; same double-join pattern as `/history` (patient INNER JOIN + clinician LEFT JOIN).
  5. `PATCH /alerts/:id` — SELECT checks existence (→ 404) and validates state transition, then UPDATE applies the change. No transaction — `db.transaction()` without `FOR UPDATE` at MySQL REPEATABLE READ adds 2 round-trips but zero race protection; alerts have no DELETE endpoint so the race is theoretical anyway.
  **Alert state machine:** forward only — `open → acknowledged → resolved`; `open → resolved` direct skip allowed. Backward transition `resolved → acknowledged` is guarded (400). `acknowledgedBy`/`acknowledgedAt` stamped only on first transition to `acknowledged` — idempotent, never overwritten. `status` is optional in `alertUpdateSchema` (Zod `.refine()` → type is `ZodEffects` not `ZodObject`) — PATCH with only `clinicianNote` is valid.
  **Drizzle `alias` import:** `alias` for MySQL table aliasing is **not** in `drizzle-orm` — it's in `drizzle-orm/mysql-core`. `import { alias } from "drizzle-orm"` throws `SyntaxError: Export named 'alias' not found` at runtime. Standard Drizzle query helpers (`eq`, `ne`, `desc`, `count`, etc.) all come from `drizzle-orm`; `alias` is the exception.
- **Daily digest (`scripts/daily-digest.ts`):** standalone Bun script — loads `.env` via `dotenv/config`, queries all open alerts (JOINed to patient name + email), queries all clinician users, sends one HTML email to all clinicians via Resend, then calls `pool.end()` to close DB connections before `process.exit`. Patient column shows `Name <email>` when name is set, email only otherwise — clinicians always have a contact address. The `bun run digest` npm script hardcodes `NODE_ENV=production` so it always hits the Railway DB — to run against local MySQL, call `bun run scripts/daily-digest.ts` directly (no NODE_ENV override). Resend's `from` address (`RESEND_FROM`) must be a verified domain in the Resend account; falls back to `alerts@resend.dev` (delivers only to the account owner) if unset. If no clinician accounts exist in the DB, exits 1 unless `DIGEST_FALLBACK_EMAIL` is set — in that case sends to that address instead (useful for a team inbox during initial setup).
...
- `RESEND_API_KEY` — Resend API key; required by `scripts/daily-digest.ts`
- `RESEND_FROM` — verified sender address for Resend, e.g. `Journey to Recovery <alerts@yourdomain.com>`; falls back to `alerts@resend.dev` if unset (only delivers to account owner)
- `DIGEST_FALLBACK_EMAIL` — optional; if set, the digest goes here when no clinician accounts exist in the DB instead of exiting 1
...
**GitHub Actions** needs these repo secrets (Settings → Secrets and variables → Actions):
- `.github/workflows/eval-gate.yml`, `promote-baseline.yml` — `OPENAI_API_KEY`, `GEMINI_API_KEY`. Also requires manually marking `eval-gate` as a required status check in branch protection for the regression gate to actually block merging.
- `.github/workflows/daily-digest.yml` — `RESEND_API_KEY`, `RESEND_FROM`, plus all five Railway MySQL vars (`MYSQLUSER`, `MYSQL_ROOT_PASSWORD`, `RAILWAY_TCP_PROXY_DOMAIN`, `RAILWAY_TCP_PROXY_PORT`, `MYSQL_DATABASE`). Runs at 08:00 UTC daily; also has `workflow_dispatch` for manual testing. Exits 0 (no failure) if there are no open alerts — safe to leave always-on.
...
- API endpoints are split by domain into seven router files under `packages/server/routes/` (`authRoutes.ts`, `profileRoutes.ts`, `checkinRoutes.ts`, `goalRoutes.ts`, `wellnessRoutes.ts`, `chatRoutes.ts`, `alertRoutes.ts`), each mounted independently at `/api` in `index.ts` — see the Server section above for what each owns. All 7 mount at the same `/api` prefix in a fixed order; nothing prevents two routers from claiming the same method+path — the earlier-mounted one would silently win, so check for collisions by hand when adding a route.
