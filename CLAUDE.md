...
`fakeDb.select`'s `from` result now exposes both an `innerJoin` branch (for queries that JOIN the `user` table) and a `where` branch (for non-JOIN queries) — both resolve through `dbSelectWhereResult` or the `orderBy/limit` chain as normal. The `innerJoin` extension in `_testUtils.ts` was added in the "alert history + badge count" step so that `GET /alerts`, `GET /alerts/history`, and `GET /alerts/:id` (all of which JOIN `user`) work through the same mock. Non-JOIN routes are unaffected.
...
- **Alert routes (`alertRoutes.ts`):** five routes in this fixed order (order matters — `count` and `history` must come before `/:id` or Express's param capture swallows them):
  ...
  3. `GET /alerts/history` — acknowledged + resolved alerts, JOINed to `user`, ordered by `updatedAt DESC`.
  4. `GET /alerts/:id` — single alert detail, also JOINed to `user` for `patientName`/`patientEmail`.
  5. `PATCH /alerts/:id` — update status/note; wrapped in `db.transaction()` so the existence check (SELECT) and the write (UPDATE) share the same DB connection; errors inside the transaction are thrown as `AlertRouteError` (class defined in the same file) and caught in the outer `catch` which maps them to 404/400.
