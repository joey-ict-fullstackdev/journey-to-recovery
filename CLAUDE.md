...
- **Alert state machine (`alertRoutes.ts`):** alerts move forward only — `open → acknowledged → resolved`; the schema allows `open → resolved` directly (skip). `PATCH /alerts/:id` guards the backward transition `resolved → acknowledged` (400). `acknowledgedBy`/`acknowledgedAt` are stamped only on the first transition to `acknowledged` (idempotency — a second `PATCH { status: "acknowledged" }` does not overwrite the original auditor). `status` is optional in `alertUpdateSchema` (it uses `.refine()` so its Zod type is `ZodEffects`, not `ZodObject`) — a PATCH with only `clinicianNote` and no `status` is valid and updates the note without touching state. `GET /api/alerts` returns only `status = "open"` rows; there is no history endpoint for acknowledged/resolved alerts yet.
- **Daily digest (`scripts/daily-digest.ts`):** standalone Bun script — loads `.env` via `dotenv/config`, queries all open alerts (JOINed to patient email), queries all clinician users, sends one HTML email to all clinicians via Resend, then calls `pool.end()` to close DB connections before `process.exit`. The `bun run digest` npm script hardcodes `NODE_ENV=production` so it always hits the Railway DB — to run against local MySQL, call `bun run scripts/daily-digest.ts` directly (no NODE_ENV override). Resend's `from` address (`RESEND_FROM`) must be a verified domain in the Resend account; falls back to `alerts@resend.dev` (delivers only to the account owner) if unset.
